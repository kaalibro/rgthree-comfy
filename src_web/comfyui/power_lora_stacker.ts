 ...
 */
class RgthreePowerLoraStacker extends RgthreeBaseServerNode {
  static override title = NodeTypesString.POWER_LORA_STACKER;
  static override type = NodeTypesString.POWER_LORA_STACKER;
  static comfyClass = NodeTypesString.POWER_LORA_STACKER;
  ...
  private addNewLoraWidget(lora?: string) {
    ...
    const widget = this.addCustomWidget(
      new PowerLoraStackerWidget("lora_" + this.loraWidgetsCounter),
    );
  ...
  override getSlotMenuOptions(slot: any): ContextMenuItem[] | null {
    ...
    if (slot?.widget?.name?.startsWith("lora_")) {
      const widget = slot.widget as PowerLoraStackerWidget;
...
/**
 * The PowerLoraStackerHeaderWidget that renders a toggle all switch, as well as some title info
...
class PowerLoraStackerHeaderWidget extends RgthreeBaseWidget<{ type: string }> {
  ...
  value = { type: "PowerLoraStackerHeaderWidget" };

  protected override hitAreas: RgthreeBaseHitAreas<"toggle"> = {
    toggle: { bounds: [0, 0] as Vector2, onDown: this.onToggleDown },
  };

  constructor(name: string = "PowerLoraStackerHeaderWidget") {
    super(name);
  }

  draw(
    ctx: CanvasRenderingContext2D,
    node: RgthreePowerLoraStacker,
    w: number,
    posY: number,
    height: number,
  ) {
  ...
  onToggleDown(event: AdjustedMouseEvent, pos: Vector2, node: TLGraphNode) {
    (node as RgthreePowerLoraStacker).toggleAllLoras();
    this.cancelMouseDown();
    return true;
  }
}

const DEFAULT_LORA_WIDGET_DATA: PowerLoraStackerWidgetValue = {
  on: true,
...
type PowerLoraStackerWidgetValue = {
  on: boolean;
  lora: string | null;
  strength: number;
...
const NODE_CLASS = RgthreePowerLoraStacker;

/** Register the node. */
app.registerExtension({
  name: "rgthree.PowerLoraStacker",
