...
class RgthreePowerLoraStacker extends RgthreeBaseServerNode {
    ...
    addNewLoraWidget(lora) {
        ...
        const widget = this.addCustomWidget(new PowerLoraStackerWidget("lora_" + this.loraWidgetsCounter));
        if (lora)
            widget.setLora(lora);
...
RgthreePowerLoraStacker.title = NodeTypesString.POWER_LORA_STACKER;
RgthreePowerLoraStacker.type = NodeTypesString.POWER_LORA_STACKER;
RgthreePowerLoraStacker.comfyClass = NodeTypesString.POWER_LORA_STACKER;
RgthreePowerLoraStacker[_a] = {
    type: "combo",
    values: [PROP_VALUE_SHOW_STRENGTHS_SINGLE, PROP_VALUE_SHOW_STRENGTHS_SEPARATE],
};
class PowerLoraStackerHeaderWidget extends RgthreeBaseWidget {
